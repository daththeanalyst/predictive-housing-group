# Claude Code — Benchmark Instructions

> **Read `INSTRUCTIONS.md` (in the parent folder) first.** It explains the full process.
> This file only covers Claude Code-specific setup.
...
4. When done, save the conversation log and any generated files
5. Start a **fresh** session and repeat for Run 2

### Prompt B (do this TWICE — 2 separate sessions)
1. Start a **new** Claude Code session
2. Paste the full text from `prompt_b_debugging.txt`
3. **Do not help the agent.** Don't hint at what might be wrong.
4. When done, save the conversation log and corrected code
5. Start a **fresh** session and repeat for Run 2
...
- **Save everything** — conversation logs, code, plots, terminal output
